...
class ExampleClass1 {
	...
	void method3 (int arg1) {
		// void에서 return은 강제로 메서드를 종료하기위해 사용
	}
	// 4. 매개변수 : X, 반환값 : X
	void method4 () {
	...
	// 매개변수O: 클래스 내부의 값만으로 처리를 못 할 경우 매개변수를 받아와야함
	// 매개변수X: 그 결과를 외부에서 사용하느냐 마느냐로 void 사용
...
// 잠자기 		(sleep) - 작업 : "(취침시간) ~ (기상시간)까지 취침" 출력 //void, 매개변수 받아옴
// 밥먹기 		(eat) - 작업 : "(음식) 먹음" 출력 //void, 매개변수 받아옴
// 훈련하기		(train) - 작업 : "훈련!!" 출력 //void, 매개변수 필요없음.
// 경기결과내기	(setGameResult) - 작업 : 골, 어시스트, 경고수 속성 변경 //void, 매개변수 받아옴
// 이적하기		(moveTeam) - 작업 : 팀 변경 //void, 매개변수 받아옴
...
class FootballPlayerA {
	...
	void setGameResult (int goalResult, int assistResult, int foulResult) {
		goal += goalResult; // 누적 상황시 복합대입연산자 사용
		assist += assistResult;
		foul += foulResult;
	}
...
public class A_ClassAndObject {
	...
	public static void main(String[] args) {
		...
		FootballPlayerA son = new FootballPlayerA(); // 인스턴스1 생성
		FootballPlayerA lee = new FootballPlayerA(); // 인스턴스2 생성
		...
		ExampleClass1.staticAttribute = 99; // 스테틱은 클래스에 직접 접근이 가능함
	//	ExampleClass1.attribute1 = 10; // instance 변수와 메서드는 '반드시' 인스턴스를 생성하여 인스턴스로 접근 및 호출 가능
	//	ExampleClass1.method1(0,0);  // instance 변수와 메서드는 '반드시' 인스턴스를 생성하여 인스턴스로 접근 및 호출 가능
		instance1.staticAttribute = 99;
		System.out.println(instance1.staticAttribute); // static은 1개만 설정이 되어도 다 공유함
